...
const sessionSecret = (env: CloudflareEnv) => env.SESSION_SECRET ?? 'chonchetrip-local-session-secret'
...
export const isEditorRequest = async (request: Request, env: CloudflareEnv) => {
  ...
  const expected = await sign(expiresValue, sessionSecret(env))
  return signature === expected
}

export const answerIsCorrect = (answer: unknown, env: CloudflareEnv) => {
  if (typeof answer !== 'string') return false
  const expected = env.EDITOR_CODE ?? 'до'
